...
This is a snapshot, not two-way sync. The last successful upload wins. Restore replaces what is on the phone. The [web app](https://builder.cdr.xyz) reads the same object on a laptop or iPhone if you paste the same credentials. Tasks, notes, the watchlist, and podcasts are the screens that app edits.
