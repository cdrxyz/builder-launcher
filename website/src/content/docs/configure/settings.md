...
| Home apps | `plaintext` (default) or `icons`. Names vs grayscale icons for pins and home search. All apps stays in color. **Pin time** `off` (default) or `on` shows minutes today and share of phone time under each pin, as `30m (17%)`. Green if the app is productive, red if not. Needs [usage](../use/usage/) access. |
