...
Offline, the last snapshot stays on the device. The browser only keeps a slim copy (subscriptions, playback progress, tickers, todos, notes) — episode catalogs stay in memory after RSS hydrate so Safari does not hit localStorage quota. Because nothing is kept between opens, a show reopened more than 30 minutes after its last feed check (or after any pull) refetches its feed first.
