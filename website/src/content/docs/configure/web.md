...
**Preferred:** create a builder.cdr.xyz account (email + password). Settings login fields ask the browser password manager, and a successful sign-in can save the login. Sign-in asks before it **overwrites** local data with the account snapshot. After that, sync **merges**: different tasks/notes keep both sides; the same item is last-write-wins on `updatedAt` / `completedAt` / `createdAt` (not a field-level CRDT). Task order is separate: a new task from either side lands at the top, and a manual reorder wins on `orderedAt` without dropping a newer text edit. Deletes union. **Include AI credentials** (off by default) puts the phone API key and OAuth tokens in the snapshot. After a sync, type `?` and a question. The reply opens full-screen chat. History lists past threads from the snapshot. The Worker forwards the bearer token to the provider you already configured and does not store it. A Hermes box on the LAN still has to be asked from the phone.
...
- **tasks** — full list. Checkbox completes a task: strikethrough, then after a second it fades to the top of completed. Tap a done line to reopen it. Tap open-row text to edit in the command bar. Done rows have an **X** to delete. Open rows have up and down arrows, and a long-press drag, to reorder. A new task lands at the top until you move it lower. Command bar starts in `-`.
...
- **settings** — Builder account first, **auto-sync** (off / on save / 30s / 5 min; on save is default and also pulls every 5 minutes while the tab is open), weather location and units, **Include AI credentials**, an optional browser API key, S3 collapsed underneath. Sign-in and S3 save & pull ask before overwriting local data.

Offline, the last snapshot stays on the device. The browser only keeps a slim copy (subscriptions, playback progress, tickers, todos, notes) — episode catalogs stay in memory after RSS hydrate so Safari does not hit localStorage quota. A fresh page load refetches every subscription because those catalogs are not stored. Within a session, a show is refetched when you open Podcasts or that show if the last successful check is older than 30 minutes.
