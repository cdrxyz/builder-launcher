---
title: Web
description: Read notes and tasks from the same encrypted S3 backup on a laptop or iPhone.
---
...
This is a snapshot, not two-way file sync. The last successful upload wins. Restore on the phone replaces local todos and notes with that snapshot.
...
S3 keys and the encryption key are stored in `localStorage` on that browser. **forget this device** clears them. The GitHub Pages host never sees the keys or the backup. API keys inside a snapshot are not shown in the UI.
