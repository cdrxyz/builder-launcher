...
- **No analytics.**
- **Todos, notes, stocks, and podcasts** stay in local storage on the phone unless you turn on [Backup](backup/) (Builder account two-way sync, or an encrypted S3 snapshot you own), tap **Share unencrypted JSON**, or open the [web app](web/) with the same account or S3 credentials. The Pages host never sees those keys. A Builder account stores the snapshot in R2 for that login (email and password hash stay in D1). Podcast episode HTML is omitted from the account snapshot; the app refetches show notes from RSS.
